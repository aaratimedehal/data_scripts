"""
Dex - Greenely Contract Analytics Bot (OAuth Version)
Integrates Slack with Snowflake Cortex Agent for natural language queries about contracts.
Supports OAuth installation for multiple workspaces - "Add to Slack" functionality.
"""
import os
import json
import logging
...
SLACK_CLIENT_ID = os.getenv("CLIENT_ID_DEX") or os.getenv("SLACK_CLIENT_ID")
...
# Workspace token storage file (JSON file - can upgrade to database later)
WORKSPACE_TOKENS_FILE = "workspace_tokens.json"

# Simple file-based installation store for OAuth
class FileInstallationStore:
    """Simple file-based installation store for workspace tokens"""
    ...
    def _load_data(self):
        """Load workspace tokens from file"""
        try:
    ...
    def find_installation(self, *, enterprise_id=None, team_id=None, user_id=None, is_enterprise_install=None):
        ...
        if lookup_id and lookup_id in data:
            ...
            return Installation(
                ...
                team_id=install_data.get("team_id"),
                team_name=install_data.get("team_name"),
    ...
    def delete_installation(self, *, enterprise_id=None, team_id=None, user_id=None):
        ...
        if lookup_id and lookup_id in data:
            del data[lookup_id]
            self._save_data(data)
            logger.info(f"Deleted installation for workspace: {lookup_id}")

# Simple file-based OAuth state store
class FileOAuthStateStore:
    ...
    def issue(self, *args, **kwargs):
        ...
        file_path = os.path.join(self.base_dir, state)
        with open(file_path, 'w') as f:
            json.dump({"created_at": time.time()}, f)
        return state
    
    def consume(self, state):
        """Consume (validate and delete) a state"""
        file_path = os.path.join(self.base_dir, state)
...
installation_store = FileInstallationStore(WORKSPACE_TOKENS_FILE)

# Initialize Slack app with OAuth (no token needed initially)
app = App(
    signing_secret=SLACK_SIGNING_SECRET,
    oauth_flow=OAuthFlow(settings=OAuthSettings(
        client_id=SLACK_CLIENT_ID,
        client_secret=SLACK_CLIENT_SECRET,
        scopes=["app_mentions:read", "channels:history", "chat:write", "commands", "im:history", "im:read", "im:write"],
        installation_store=installation_store,
        state_store=FileOAuthStateStore(expiration_seconds=600, base_dir="./.slack_oauth_states")
    ))
)
    ...
    return {
        "blocks": [
                ...
                "elements": [
                    {
                        ...
                        "text": {
                            "type": "plain_text",
                            "text": "📈 Customers signed this year"
                        },
                        "action_id": "quick_question_1",
                        "value": "How many contracts were signed this year?"
                    },
                    {
                        "type": "button",
                        "text": {
                        ...
                        "value": "How many customers churned last month?"
                    },
                    {
                        "type": "button",
                        "text": {
                            "type": "plain_text",
                            "text": "🇸🇪 Top performing Swedish Channels "
                        },
                        "action_id": "quick_question_3",
                        "value": "Which channels brought in the most number of customers in Sweden last quarter ? "
                    }
                ]
            },
            {
                ...
                "elements": [
                    {
                        ...
                        "text": {
                            "type": "plain_text",
                            "text": ":electric_plug: EV & Charging Station Customers"
                        },
                        "action_id": "quick_question_4",
                        "value": "How many users that signed in 2025 have both an EV and Charging Station connected?"
                    },
                    {
                        "type": "button",
                        "text": {
                            "type": "plain_text",
                            "text": ":bulb: Rörligt customers"
                        },
                        "action_id": "quick_question_5",
                        "value": "Which channels drove the most variable-price contracts in January 2026?"
                    },
                    {
                        "type": "button",
                        "text": {
                            "type": "plain_text",
                            "text": ":credit_card: Autogiro customers"
                        },
                        "action_id": "quick_question_6",
                        "value": ":What share of Swedish customers use autogiro today?"
                    }
                ]
            },
            {
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "*💡 Tips:*\n• Be specific with time periods (e.g., 'this month', 'last quarter', '2025')\n• I'll show you the SQL query used if you click 'Show Query Details'\n• Processing typically takes 20-30 seconds (based on query complexity)\n• Use `/dex-help` anytime to see this message again \n• You can continue the conversation in the same thread by replying to my message \n• If you see 'dispatch_failed', please wait a few seconds - your question is still being processed, don't send it again!" 
                }
            },
            {
                "type": "context",
...
def handle_mention(event, say):
    ...
    Supports both regular mentions and thread replies.
    """
    text = event.get("text", "").strip()
    
    # Check if mention is in a thread
    thread_ts = event.get("thread_ts") or event.get("ts")  # Use thread_ts if in thread, otherwise reply to the mention itself
    ...
    if not text:
        ...
        welcome_msg = get_welcome_message()
        say(blocks=welcome_msg["blocks"], text="Dex Help - Contract Analytics Assistant", thread_ts=thread_ts)
        return
    ...
    # Post in thread (if in a thread) or as reply to the mention
    if blocks:
        say(blocks=blocks, text=text_fallback, thread_ts=thread_ts)
    else:
        say(text_fallback, thread_ts=thread_ts)
...
def handle_dex_command(ack, respond, command):
    ...
    try:
        ...
        response_url = command.get("response_url")
        # Check if command was invoked in a thread by checking if response_url exists
        # Note: Slack doesn't provide thread_ts in slash command payloads, but response_url
        # can be used to respond in the thread context
        is_in_thread = response_url is not None
        ...
        if not question:
            ...
            try:
                ...
                # If in thread, use response_url; otherwise use respond()
                if is_in_thread and response_url:
                    # Use response_url to post in thread
                    requests.post(
                        response_url,
                        json={
                            "blocks": welcome_msg["blocks"],
        ...
        if not AGENT_ENDPOINT or not PAT:
            ...
            try:
                ...
                # If in thread, use response_url; otherwise use chat_postMessage
                initial_blocks = [
                    {
                        "type": "section",
                        "text": {
                ...
                if is_in_thread and response_url:
                    # Use response_url to post in thread (this will automatically post in thread context)
                    ...
                    initial_response_data = {"ts": None, "channel": command.get("channel_id")}
                    if initial_response.status_code == 200:
                        try:
                            response_json = initial_response.json()
                            initial_response_data["ts"] = response_json.get("ts")
                        except:
                            pass
                    initial_message = initial_response_data
                else:
                    # Post normally using chat_postMessage
                    initial_message = app.client.chat_postMessage(
                        channel=command.get("channel_id"),
                        text=f"Your question: {question}\n\nProcessing...",
                        blocks=initial_blocks
                    )
                # Extract message info for updates
                if isinstance(initial_message, dict):
                    initial_response = {"ts": initial_message.get("ts"), "channel": initial_message.get("channel")}
                else:
                    initial_response = {"ts": initial_message.get("ts"), "channel": initial_message.get("channel")} if initial_message else None
                ...
                try:
                    if is_in_thread and response_url:
                        # When in thread, we can't update the message, so post a new one in the thread
                        # Use response_url again to post in the same thread
                        response_payload = {
                            "blocks": blocks if blocks else None,
                            "text": text_fallback,
                            "response_type": "in_channel"
                    ...
                    elif initial_response and 'ts' in initial_response and initial_response['ts']:
                        # Update the processing message with the final response (non-thread case)
                        ...
                        if blocks:
                            respond(blocks=blocks, text=text_fallback, response_type="in_channel")
                        else:
                            respond(text=text_fallback, response_type="in_channel")
                except Exception as update_error:
                    # If update fails, send as new message
                    logger.error(f"Could not update message: {update_error}", exc_info=True)
                    if is_in_thread and response_url:
                        # Post in thread using response_url
                        response_payload = {
                            "blocks": blocks if blocks else None,
                            "text": text_fallback,
                            "response_type": "in_channel"
        ...
        try:
            ...
            response_url = command.get("response_url")
            if response_url:
                # Post in thread if command was in thread
                requests.post(
                    response_url,
                    json={
                        "text": error_msg,
...
# SlackRequestHandler automatically handles /slack/install and /slack/oauth_redirect
# when OAuthFlow is configured - it manages state properly
...
def index():
    ...
    <html>
    <head>
        <title>Dex - Contract Analytics Bot</title>
        <style>
            body {{
    ...
    <body>
        ...
        <p>Your AI assistant for contract analytics powered by Snowflake Cortex Agent.</p>
        <p>Click the button below to add Dex to your Slack workspace:</p>
        <a href="{install_url}" class="button">Add to Slack</a>
    </body>
    </html>
    """
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Dex OAuth Server (Greenely Contract Analytics Bot)...")
    print(f"📡 Server URL: {SLACK_BOT_URL}")
    print(f"🔗 Installation URL: {SLACK_BOT_URL}/slack/install")
    print(f"🎯 Events URL: {SLACK_BOT_URL}/slack/events")
    print(f"🔄 OAuth Redirect: {SLACK_BOT_URL}/slack/oauth_redirect")
    print("\n💬 Bot will respond to:")
    ...
    print(f"\n⚡️ Bot is running on port {port}!")
    print("\n💡 Share this URL with others to install Dex in their workspace:")
